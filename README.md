...
* **Extreme I/O & Memory Control**: Replaced legacy line-by-line file reading with **Block-I/O** strategies to minimize disk latency. Eradicated Out-Of-Memory (OOM) failures during massive patch merging, **3D phase unwrapping, and SBAS inversion** (e.g., >30M points) via novel **Streamed Pre-allocation, Row-Block Streaming, and Two-Phase Gridding** architectures, locking peak memory usage to the theoretical minimum.
...
| **Step 5.5 (`ps_merge_patches`)**| OOM Crash | **Stable** | **Memory Bound** | Streamed Pre-allocation, Global Inverse Mapping, Chunked Masking |
| **Step 6 (`ps_unwrap`)** | 1068s | **411s** | **2.6x** | **Row-Block Streaming, Two-Phase Gridding**, Parallel Execution, Snaphu Optimization |
