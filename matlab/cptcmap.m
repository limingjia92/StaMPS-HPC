...
% not supported yet (StaMPS-HPC: X11 color names is supported)
%
% Input variables:
%
...
% Mingjia Li        03/2026     Robust path resolution via 'mfilename'. Upgraded parser to safely handle 
%                               '/' & '-' evaluation bugs, translate X11 colors, and expand 4-col categorical data.
%------------------------------
% Parse input
%------------------------------

% Set up .cpt file folder
% [StaMPS-HPC]: Use mfilename for absolute robustness and resolve parent directory
script_path = mfilename('fullpath');          
[script_dir, ~, ~] = fileparts(script_path);  
[parent_dir, ~, ~] = fileparts(script_dir);   
cptpath = fullfile(parent_dir, 'cptfiles', filesep); 

if ~exist(cptpath, 'dir')
    error('StaMPS_HPC:MissingDir', 'Cannot find CPT dir, make sure it in path: %s', cptpath);
end
...
Opt = struct('mapping', 'scaled', ...
             'ncol', NaN, ...
             'flip', false, ...
             'clip_neg', false); % [StaMPS-HPC]: cut negtive value
...
[cmap, lims,ticks,bfncol] = cpt2cmap(filename, Opt.ncol, Opt.clip_neg);
if Opt.flip
...
function [cmap, lims, ticks, bfncol] = cpt2cmap(file, ncol, clip_neg)
...
% =======================================================================
% [StaMPS-HPC]: Universal Parser Enhancements
% =======================================================================
% 1. Replace '/' with space to prevent 'str2num' from evaluating as division.
ctabletxt = regexprep(ctabletxt, '/', ' ');

% 2. Replace '-' separator in HSV format (e.g., 360-1-1) with space.
% Note: Using lookahead/lookbehind regex to only replace hyphens between 
% numbers, safely preserving legitimate negative signs (e.g., -100).
ctabletxt = regexprep(ctabletxt, '(?<=\d)-(?=\d)', ' ');

% 3. Intercept and translate standard X11 color names to RGB strings.
color_dict = {
    'black', '0 0 0'; 'white', '255 255 255'; 'gray', '128 128 128';
    'red', '255 0 0'; 'green', '0 255 0'; 'blue', '0 0 255';
    'yellow', '255 255 0'; 'cyan', '0 255 255'; 'magenta', '255 0 255';
    'mediumaquamarine', '102 205 170'; 'mediumseagreen', '60 179 113';
    'burlywood', '222 184 135'; 'darkorange3', '205 102 0';
    'saddlebrown', '139 69 19'; 'bisque2', '238 213 210';
    'ivory2', '238 238 224'; 'snow', '255 250 250'
};
for k = 1:size(color_dict, 1)
    ctabletxt = regexprep(ctabletxt, ['\<' color_dict{k,1} '\>'], color_dict{k,2}, 'ignorecase');
end

% Safely convert the processed text to a numeric matrix.
ctable = str2num(strvcat(ctabletxt));

% 4. [Dimensionality Expansion] Automatically handle 4-column Categorical 
% data, expanding it into an 8-column stepwise continuous block.
if ~isempty(ctable) && size(ctable, 2) == 4
    nr_cat = size(ctable, 1);
    new_ctable = zeros(nr_cat, 8);
    for ir = 1:nr_cat-1
        % Assemble format: [StartVal R G B, EndVal(NextStart) R G B]
        new_ctable(ir, :) = [ctable(ir,1:4), ctable(ir+1,1), ctable(ir,2:4)];
    end
    % Predict the final boundary step for the last block
    step = ctable(end,1) - ctable(end-1,1);
    new_ctable(end, :) = [ctable(end,1:4), ctable(end,1)+step, ctable(end,2:4)];
    ctable = new_ctable;
end
% =======================================================================
...
for iline = 1:length(footer)
    % [StaMPS-HPC]: Added safe parsing to ignore X11 color strings (e.g., 'black')
    % since these background parameters are overridden by ps_plot_ifg_hpc anyway.
    if strcmp(footer{iline}(1), 'B')
        val = str2num(regexprep(footer{iline}, 'B', ''));
        if length(val) == 3, bfncol(1,:) = val; end
    elseif strcmp(footer{iline}(1), 'F')
        val = str2num(regexprep(footer{iline}, 'F', ''));
        if length(val) == 3, bfncol(2,:) = val; end
    elseif strcmp(footer{iline}(1), 'N')
        val = str2num(regexprep(footer{iline}, 'N', ''));
        if length(val) == 3, bfncol(3,:) = val; end
    end
end
...
% =======================================================================
% [StaMPS-HPC]: cut negative value for ocean
if nargin >= 3 && clip_neg && lims(1) < 0 && lims(2) > 0
    lims(1) = 0; 
end
% =======================================================================
