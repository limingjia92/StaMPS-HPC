...
* **`ps_gacos2tca`**: A standalone utility that directly assimilates GACOS tropospheric delay data (GeoTIFF/Binary) into StaMPS-HPC. It natively computes the TCA phase matrix for both PS and SBAS modes, entirely eliminating the reliance on heavy external toolboxes like TRAIN.